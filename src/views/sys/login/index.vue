    ...
    Col(span="8")
      .mt-10.ml-10.mr-10.enter-x
        Tabs(v-model:activeKey="activeKey" :class="`${prefixCls}-tabs`" )
          TabPane(key="login" :tab="t('sys.login.accountLogin')" v-if="activeKey === 'login' || activeKey === 'textlogin'")
            LoginForm(@toggleTabs="toggleTabs")
          TabPane(key="textlogin" :tab="t('sys.login.textLogin')" v-if="activeKey === 'login' || activeKey === 'textlogin'")
            TextLogin(@toggleTabs="toggleTabs")
          TabPane(key="forget" :tab="t('sys.login.forgetFormTitle')" v-if="activeKey === 'forget'")
            ForgetPasswordForm(@toggleTabs="toggleTabs")
          TabPane(key="regist" :tab="t('sys.login.signUpFormTitle')" v-if="activeKey === 'regist'")
